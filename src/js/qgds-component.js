...
const QGDSComponent = {
  ...
  make: function (name, opts = {}) {
    let componentPath =
				opts.path ||
				`./src/components/${this.framework}/${name}/${name}.html`,
    ...
    let 
      ...
      customdataJSON = {},
      templatedata = {},
      defaultDatafile = `./src/components/${this.framework}/${name}/data.json`;

    //Does default data file exist (/component/data.json) ?
    if (fs.existsSync(defaultDatafile)) {
      try {
        defaultfileJSON = JSON.parse(
          fs.readFileSync(defaultDatafile, "UTF-8"),
        );
      } catch (err) {
        console.error(
          `Error parsing datafile for ${name}: ${defaultDatafile}, ${err}`,
        );
      }
    }
