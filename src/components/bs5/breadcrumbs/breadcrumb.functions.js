...
export function initBreadcrumb() {
  ...
  breadcrumb.forEach((item) => {
    let breadcrumbList = item.querySelectorAll(".breadcrumb-item");

    // Return when breadcrumb does not exist
    // and is greater than maxLength.
    if (!breadcrumbList || !breadcrumbList.length || breadcrumbList.length <= maxLength) {
      return;
    }
    // Shorten breadcrumb.
